...
// ── Helper: safely convert string to Tibetan numeral ─────────────────────────
String _tibStr(String s) {
  final n = int.tryParse(s);
  return n != null ? toTibNum(n) : s;
}
...
class _MilestoneCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final countdownText = daysUntil == 0
        ...
        : daysUntil == 1
            ? s('In 1 Day', 'ཉིན་༡ ནང་')
            : s('In $daysUntil Days', 'ཉིན་${toTibNum(daysUntil)} ནང་');
    ...
    final lunarLabel = day.tibetanDayEn.isNotEmpty && day.tibetanMonthEn.isNotEmpty
        ? s(
            ...
            'ཉིན་${_tibStr(day.tibetanDayEn)} · ཟླ་${_tibStr(day.tibetanMonthEn)}',
          )
        : '';
                ...
                return GestureDetector(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Month abbrev
                      Text(
                        bo ? tibMonthShort(d.month) : _monthShort[d.month],
                        style: TextStyle(
                          fontSize: bo ? 8 : 9,
                      ...
                      Container(
                        ...
                        alignment: Alignment.center,
                        child: Text(
                          bo ? toTibNum(d.day) : d.day.toString(),
                          style: TextStyle(
                            fontSize: 15,
                            fontWeight: FontWeight.w700,
                            color: isSel
...
class _SignificantDateRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title     = bo ? day.nameBo : day.nameEn;
    final solarDate = bo
        ? '${tibMonthFull(day.date.month)} ${toTibNum(day.date.day)}། ${toTibNum(day.date.year)}'
    ...
    final lunarDate = day.tibetanMonthEn.isNotEmpty && day.tibetanDayEn.isNotEmpty
        ? s(
            ...
            'ཟླ་${_tibStr(day.tibetanMonthEn)} · ཉིན་${_tibStr(day.tibetanDayEn)}',
          )
        : '';
    ...
    final badge = daysUntil == 0
        ...
        : daysUntil == 1
            ? s('Tomorrow', 'སང་ཉིན་')
            : s('${daysUntil}d', '${toTibNum(daysUntil)}ཉིན་');
